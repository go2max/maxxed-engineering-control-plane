...
export const RepairAction = Object.freeze({
  ACCEPT: 'ACCEPT',
...
export class RepairController {
  ...
  decide(taskKey, verification) {
    ...
    if (verification.verdict === Verdict.ESCALATE) {
      return { action: RepairAction.ESCALATE, reason: verification.reason, history: structuredClone(history) };
    }
    if (verification.verdict === Verdict.REJECT) {
      return { action: RepairAction.TERMINATE, reason: verification.reason, history: structuredClone(history) };
    }
  ...
  get(taskKey) {
    const history = this.history.get(taskKey);
    return history ? structuredClone(history) : null;
  }

  reset(taskKey) {
    return this.history.delete(taskKey);
  }
}
