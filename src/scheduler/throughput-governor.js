export class ThroughputGovernor {
  ...
  target({ availableCapacity = 0, verifierBacklog = 0, recentAccepted = 0, recentFailed = 0, degraded = false } = {}) {
    ...
    if (Number(verifierBacklog) > this.maxVerifierBacklog) { allowed = Math.min(allowed, this.baselineConcurrency); reasons.push('verifier-backlog'); }
    ...
    return {
      ...
      failureRate,
      throttled: allowed < Math.min(desired, Number(availableCapacity)),
      reasons
    };
  }
}
